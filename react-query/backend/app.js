...
app.get("/events/:id", async (req, res) => {
  ...
  if (!event) {
    return res.status(404).json({ message: `For the id ${id}, no event could be found.` });
  }
...
app.post("/events", async (req, res) => {
  ...
  if (!event.title?.trim() || !event.description?.trim() || !event.date?.trim() || !event.time?.trim() || !event.image?.trim() || !event.location?.trim()) {
