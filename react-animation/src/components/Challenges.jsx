      ...
      <motion.ol className="challenge-items" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ y: -30, opacity: 0 }} key="list">
       <AnimatePresence>
        {displayedChallenges.map((challenge) => (
         <ChallengeItem key={challenge.id} challenge={challenge} onViewDetails={() => handleViewDetails(challenge.id)} isExpanded={expanded === challenge.id} />
        ))}
       </AnimatePresence>
      </motion.ol>
     )}
     {displayedChallenges.length === 0 && (
      <motion.p key="fallback" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
       No challenges found.
      </motion.p>
     )}
    </AnimatePresence>
